...
export default async function Postcard({post}: PostcardProps): Promise<React.JSX.Element> {
    ...
    return (
        <div className="flex flex-col bg-[#f5f5f5] w-80 rounded-b-xl rounded-t-xl shadow-2xl hover:shadow-xl transition-shadow duration-300 h-120">
            <div className="h-3/5">
...
/>
                ...
                {categories.map((category) => (
                    category.id === post.category_id && (
                        <div key={category.id} className="bg-[#007bff] p-2 rounded-2xl  flex items-center justify-center">
                          ...
                          <p className="text-sm  text-white font-bold hover:underline hover:text-lg">{category.name}</p>
                        
                        </Link></div>
                    )
                ))}
                <div className="text-sm text-[#1e1e1e] font-semibold ">
      ...
      day: 'numeric'
            ...
            <div dangerouslySetInnerHTML={{ __html: post.excerpt as string }} className="px-4 py-2 text-gray-700 text-left  text-sm leading-relaxed" />
